...
filter(day_trk, id == "#399745060_2017/09") %>% summarise(length = length(x_))
filter(day_trk, id == "#399749318_2017/09") %>% summarise(length = length(x_))
...
kde_shape_1 <- trk2 %>% filter(id == "#399745060_2017/09") %>%
  hr_kde(., levels = c(0.95))
kde_shape_2 <- trk2 %>% filter(id == "#399745060_2017/10") %>%
  hr_kde(., levels = c(0.95))
...
write.csv(data_summary, file = "summary/ralph_data_month_summary.csv", row.names = FALSE)
