...
filter(day_trk, id == "X023") %>% summarise(length = length(x_))
filter(day_trk, id == "X027") %>% summarise(length = length(x_))
...
kde_shape_1 <- trk2 %>% filter(id == "X023") %>%
  hr_kde(., levels = c(0.95))
kde_shape_2 <- trk2 %>% filter(id == "X027") %>%
  hr_kde(., levels = c(0.95))
...
write.csv(data_summary, file = "summary/morgan_data_summary.csv", row.names = FALSE)
