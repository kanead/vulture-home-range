...
# write.csv(x = trk_albers, file = "regularised/galligan_albers_reg.csv", row.names = FALSE)
...
kde_shape_1 <- trk_albers %>% filter(id == "lf143660") %>%
  hr_kde(., levels = c(0.95))
kde_shape_2 <- trk_albers %>% filter(id == "wb143657") %>%
  hr_kde(., levels = c(0.95))
...
data_summary <- read_csv("summary/galligan_data_summary.csv", col_names = T)
...
write.table(data_summary, "summary/galligan_data_summary.csv", sep = ",", row.names=F)
