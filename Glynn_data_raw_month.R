...
filter(day_trk, id == "128502_2014/02") %>% summarise(length = length(x_))
filter(day_trk, id == "128502_2014/03") %>% summarise(length = length(x_))
...
kde_shape_1 <- trk2 %>% filter(id == "1120042_2016/08") %>%
  hr_kde(., levels = c(0.95))
kde_shape_2 <- trk2 %>% filter(id == "1120042_2016/09") %>%
  hr_kde(., levels = c(0.95))
...
data_summary$species<-ifelse((data_summary$bird == "137296") | (data_summary$bird == "112004"), "cv", "wb")
data_summary

#' can export this data summary
write.csv(data_summary, file = "summary/glynn_data_month_summary.csv", row.names = FALSE)
