...
filter(day_trk, id == "ID1") %>% summarise(length = length(x_))
filter(day_trk, id == "ID2") %>% summarise(length = length(x_))
...
#' this is how I cleaned up the shape file
...
#  sf::st_buffer(merged_Africa_tranform, dist = 0)

#' export this to use later
# st_write(
#  merged_Africa_tranform,
#  "C:\\Users\\Adam\\Documents\\Science\\Methods & Stats\\GIS\\merged_Africa_protected_clean.shp"
# )
...
kde_shape_1_95 <- trk2 %>% filter(id == "ID1") %>%
  hr_kde(., levels = c(0.95))
kde_shape_2_95 <- trk2 %>% filter(id == "ID2") %>%
  hr_kde(., levels = c(0.95))

#' check that areas match
hr_area(kde_shape_1_95) / 1000000
hr_area(kde_shape_2_95) / 1000000

#' plot the KDEs
plot(kde_shape_1_95$ud)
plot(kde_shape_2_95$ud)
...
#' writeRaster(kde_shape_1$ud, "KDE_ID1_95_raw.tif")
#' writeRaster(kde_shape_2$ud, "KDE_ID2_95_raw.tif")
...
#' sanity check that the area of overlap is not greater than the size of the HRs
data_summary$kde_95_raw > as.numeric(data_summary$kde_95_overlap)
data_summary$kde_50_raw > as.numeric(data_summary$kde_50_overlap)

#' can export this data summary
write.csv(data_summary, file = "summary/eswatini_data_summary.csv", row.names = FALSE)

#### END CODE ----

#' measure the intersection in alternative way 

#' convert raster to polygon
kde_shape_1_95_poly <- hr_isopleths(kde_shape_1_95)
kde_shape_2_95_poly <- hr_isopleths(kde_shape_2_95)
kde_shape_1_50_poly <- hr_isopleths(kde_shape_1_50)
kde_shape_2_50_poly <- hr_isopleths(kde_shape_2_50)

intersection_1_95 <-
  st_intersection(kde_shape_1_95_poly$geometry,
                  merged_Africa_tranform$geometry)
sum(st_area(intersection_1_95)) / 1e6

intersection_2_95 <-
  st_intersection(kde_shape_2_95_poly$geometry,
                  merged_Africa_tranform$geometry)
sum(st_area(intersection_2_95)) / 1e6

intersection_1_50 <-
  st_intersection(kde_shape_1_50_poly$geometry,
                  merged_Africa_tranform$geometry)
sum(st_area(intersection_1_50)) / 1e6

intersection_2_50 <-
  st_intersection(kde_shape_2_50_poly$geometry,
                  merged_Africa_tranform$geometry)
sum(st_area(intersection_2_50)) / 1e6

overlap_kde_95 <- c(st_area(intersection_1_95)/1e6, st_area(intersection_2_95)/1e6);overlap_kde_95
overlap_kde_50 <- c(st_area(intersection_1_50)/1e6, st_area(intersection_2_50)/1e6);overlap_kde_50
