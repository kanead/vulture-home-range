...
####' 42 indiviuals only using AWBVs
...
hv <- filter(mydata, species == "hv")
hv <- levels(as.factor(hv$id))
wh <- filter(mydata, species == "wh")
wh <- levels(as.factor(wh$id))
...
trk3 <-
  dplyr::select(trk3, x, y, date, ID) %>% arrange(as.numeric(ID), date)
head(trk3)
tail(trk3)
class(trk3$date)
levels(as.factor(trk3$ID))
...
kde_shape_1 <- trk_albers %>% filter(id == "#109018692_2015/10") %>%
  hr_kde(., levels = c(0.95))
kde_shape_2 <- trk_albers %>% filter(id == "#143101591_2016/03") %>%
  hr_kde(., levels = c(0.95))
...
data_summary <-
  read_csv("summary/kendall_tanz_data_month_summary.csv", col_names = T)
...
data_summary$species  <- ifelse(data_summary$bird %in% wh, "wh", data_summary$species)
...
write.table(
  data_summary,
  "summary/kendall_tanz_data_month_summary.csv",
  sep = ",",
  row.names = F
)
