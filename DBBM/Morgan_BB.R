...
#' #' remove track causing issues
#' levels(as.factor(trk1$id))
#' trk1 <- trk1 %>% dplyr::filter(id == "X023")
...
writeRaster(dbbmm, filename='brownian_bridges/morgan_dbbm.tif', overwrite=TRUE)
writeRaster(dbbmm, filename='brownian_bridges/morgan_dbbm', overwrite=TRUE)
...
write.csv(export_data, file = "summary/brownian/morgan_bb.csv")
