...
####' load the data ----
trk1 <- read_csv("regularised/kendall_tanz_albers_reg_month.csv", col_names = TRUE)
trk1

levels(as.factor(trk1$id))
# trk1 <- trk1 %>% dplyr::filter(!id == "#834450834" & !id == "#834451702" & !id == "#623187113") %>% droplevels()
levels(as.factor(trk1$id))
...
levels(as.factor(trk1$id))
export_data$ID <- sub(pattern = "\\.", replacement = "", x = export_data$ID)
export_data
export_data$ID <- gsub(pattern = "X", replacement = "#", x = export_data$ID)
export_data$ID <- gsub(pattern = "\\.", replacement = "/", x = export_data$ID)
export_data

write.csv(export_data, file = "summary/brownian/kendall_tanz_bb_reg_month_raster.csv", row.names = F)
