...
#' #' one factor level is causing an issue
#' #' let's drop it
#' levels(as.factor(mydata$id))
#' # mydata <- mydata %>% dplyr::filter(!id %in% c("Heidi")) %>% droplevels()
#' mydata <- mydata %>% dplyr::filter(!id == "Heidi" & !id == "Yoki") %>% droplevels()
#' levels(as.factor(mydata$id))
...
writeRaster(dbbmm, filename='brownian_bridges/schabo_dbbm.tif', overwrite=TRUE)
writeRaster(dbbmm, filename='brownian_bridges/schabo_dbbm', overwrite=TRUE)
...
#' try one
st_area(hr_isopleths(dbbmm$Cerhu, level = 0.95)) / 1e6 #' 58023.98 
st_area(hr_isopleths(dbbmm$Jakob, level = 0.95)) / 1e6 #' ERROR
...
#' this is at 95%
#' should be 948.738 for Cerhu
#' test it here
intersection_1_95 <-
  st_intersection(hr_isopleths(dbbmm$Cerhu, level = 0.95),
                  merged_Africa_tranform$geometry)
sum(st_area(intersection_1_95)) / 1e6 #' 948.738 
...
write.csv(export_data, file = "summary/brownian/schabo_bb.csv")
