...
#' data is on the order of ~ 1 hour
...
writeRaster(dbbmm, filename='brownian_bridges/mend_dbbm.tif', overwrite=TRUE)
writeRaster(dbbmm, filename='brownian_bridges/mend_dbbm', overwrite=TRUE)
...
#' try one
st_area(hr_isopleths(dbbmm$CV144780, level = 0.95)) / 1e6 #' 22691.59 
st_area(hr_isopleths(dbbmm$CV244781, level = 0.95)) / 1e6 #' 40623.83 
...
#' this is at 95%
#' should be 1957.791 for CV144780
#' test it here
intersection_1_95 <-
  st_intersection(hr_isopleths(dbbmm$CV144780, level = 0.95),
                  merged_Africa_tranform$geometry)
sum(st_area(intersection_1_95)) / 1e6 #' 1957.791 
...
write.csv(export_data, file = "summary/brownian/mend_bb.csv")
