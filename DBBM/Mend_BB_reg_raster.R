...
####' load the data ----
trk1 <- read_csv("regularised/mend_albers_reg.csv", col_names = TRUE)
trk1

levels(as.factor(trk1$id))
trk1 <- trk1 %>% dplyr::filter(!id == "CV644782" & !id == "CV353230" & !id == "CV762095" & !id == "CV453229" & !id == "CV544780") %>% droplevels()
levels(as.factor(trk1$id))
...
write.csv(export_data, file = "summary/brownian/mend_bb_reg_raster.csv")
