...
####' load the data ----
trk1 <- read_csv("regularised/schabo_albers_reg.csv", col_names = TRUE)
trk1

levels(as.factor(trk1$id))
trk1 <- trk1 %>% dplyr::filter(!id == "Heidi") %>% droplevels()
levels(as.factor(trk1$id))
...
write.csv(export_data, file = "summary/brownian/schabo_bb_reg_raster.csv", row.names = F)
