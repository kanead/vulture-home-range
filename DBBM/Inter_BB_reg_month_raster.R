...
####' load the data ----
trk1 <- read_csv("regularised/inter_albers_reg_month.csv", col_names = TRUE)
trk1


levels(as.factor(trk1$id))
length(levels(as.factor(trk1$id)))
# exclude <-
#   c("WH2417_2017/07")
# 
# trk1 <- trk1 %>% dplyr::filter(!id %in% exclude)
length(levels(as.factor(trk1$id)))
...
write.csv(export_data, file = "summary/brownian/inter_bb_reg_month_raster.csv", row.names = F)
