...
writeRaster(dbbmm, filename='brownian_bridges/lizanne_dbbm.tif', overwrite=TRUE)
writeRaster(dbbmm, filename='brownian_bridges/lizanne_dbbm', overwrite=TRUE)
...
#' try one
st_area(hr_isopleths(dbbmm$VULT01, level = 0.95)) / 1e6 #' 21680.57 
st_area(hr_isopleths(dbbmm$VULT07, level = 0.95)) / 1e6 #' ERROR
st_area(hr_isopleths(dbbmm$VULT14, level = 0.95)) / 1e6 #' ERROR
...
#' this is at 95%
#' should be 6960.932 for VUL01
#' test it here
intersection_1_95 <-
  st_intersection(hr_isopleths(dbbmm$VULT01, level = 0.95),
                  merged_Africa_tranform$geometry)
sum(st_area(intersection_1_95)) / 1e6 #' 6960.932 
