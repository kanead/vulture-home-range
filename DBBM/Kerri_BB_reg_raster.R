...
####' load the data ----
trk1 <- read_csv("regularised/kerri_albers_reg.csv", col_names = TRUE, col_types = cols(
  id = col_character()))
trk1

levels(as.factor(trk1$id))
trk1 <- trk1 %>% dplyr::filter(!id == "AG351" & !id == "AM227" & !id == "AM86" & !id == "33640" & !id == "33798" & !id == "27233665" & !id == "Ingelheim") %>% droplevels()
levels(as.factor(trk1$id))
...
write.csv(export_data, file = "summary/brownian/kerri_bb_reg_raster.csv")
