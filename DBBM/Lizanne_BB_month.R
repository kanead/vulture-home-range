...
trk1 %>% group_by(id) %>% summarise(count=n())
 ...
 exclude <-
   c("VULT05_2014/06", "VULT09_2014/06", "VULT09_2014/12", "VULT24_2014/07")

trk1 <- trk1 %>% dplyr::filter(!id %in% exclude) %>% droplevels()
levels(as.factor(trk1$id))
...
#' try one
st_area(hr_isopleths(dbbmm$VULT01_2013.12, level = 0.95)) / 1e6 #' 6839.758 
st_area(hr_isopleths(dbbmm$VULT01_2014.02, level = 0.95)) / 1e6 #' 23186.58 
...
#' test it here
intersection_1_95 <-
  st_intersection(hr_isopleths(dbbmm$VULT01_2013.12, level = 0.95),
...
write.csv(export_data, file = "summary/brownian/lizanne_bb_month.csv")
