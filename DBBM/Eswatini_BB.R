...
writeRaster(dbbmm, filename='brownian_bridges/swazi_dbbm', overwrite=TRUE)
...
#' try one
st_area(hr_isopleths(dbbmm$ID1, level = 0.95)) / 1e6 #' 6997.363 
st_area(hr_isopleths(dbbmm$ID2, level = 0.95)) / 1e6 #' 26910.8
...
write.csv(export_data, file = "summary/brownian/eswatini_bb_raster.csv")
...
ID1BB <- hr_isopleths(dbbmm$ID1, level = 0.95)
ID2BB <- hr_isopleths(dbbmm$ID2, level = 0.95)
st_area(ID1BB) / 1e6 #' 2414.935km2
st_area(ID2BB) / 1e6 #' 14683.24km2
...
#' this is at 95%
intersection_1_95 <-
  st_intersection(ID1BB,
                  merged_Africa_tranform$geometry)
sum(st_area(intersection_1_95)) / 1e6 #' 368.4835km2

intersection_2_95 <-
  st_intersection(ID2BB,
                  merged_Africa_tranform$geometry)
sum(st_area(intersection_2_95)) / 1e6 #' 10395.06km2
