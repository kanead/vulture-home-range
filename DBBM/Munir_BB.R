...
writeRaster(dbbmm, filename='brownian_bridges/munir_dbbm.tif', overwrite=TRUE)
writeRaster(dbbmm, filename='brownian_bridges/munir_dbbm', overwrite=TRUE)
...
#' try one
st_area(hr_isopleths(dbbmm$X171604adWBV05, level = 0.95)) / 1e6 #' 23667.99 
st_area(hr_isopleths(dbbmm$X171605Matira, level = 0.95)) / 1e6 #' 13695.12
...
#' this is at 95%
#' should be 17044.32 for X171604adWBV05
#' test it here
intersection_1_95 <-
  st_intersection(hr_isopleths(dbbmm$X171604adWBV05, level = 0.95),
                  merged_Africa_tranform$geometry)
sum(st_area(intersection_1_95)) / 1e6 #' 17044.32 
...
write.csv(export_data, file = "summary/brownian/munir_bb.csv")
