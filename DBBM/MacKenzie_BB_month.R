...
trk1 %>% group_by(id) %>% summarise(count=n())
 ...
 exclude <-
   c("5863wb_2018/09", "6220wh_2018/11", "6484wh_2019/10")
...
#' try one
st_area(hr_isopleths(dbbmm$X5403wb_2017.05, level = 0.95)) / 1e6 #' 6839.758 
st_area(hr_isopleths(dbbmm$X5403wb_2017.06, level = 0.95)) / 1e6 #' 23186.58 
...
#' test it here
intersection_1_95 <-
  st_intersection(hr_isopleths(dbbmm$X5403wb_2017.05, level = 0.95),
...
write.csv(export_data, file = "summary/brownian/mackenzie_bb_month.csv")
