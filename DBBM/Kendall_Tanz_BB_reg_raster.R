...
####' load the data ----
trk1 <- read_csv("regularised/kendall_tanz_albers_reg.csv", col_names = TRUE)
trk1

levels(as.factor(trk1$id))
trk1 <- trk1 %>% dplyr::filter(!id == "#834450834" & !id == "#834451702" & !id == "#623187113" & !id == "#834449829") %>% droplevels()
levels(as.factor(trk1$id))
...
write.csv(export_data, file = "summary/brownian/kendall_tanz_bb_reg_raster.csv")
