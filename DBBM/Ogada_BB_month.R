...
#' add the bird names to the data frame
trk1$Bird <- gsub("(.+?)(\\_.*)", "\\1", trk1$id)

levels(as.factor(trk1$id))
levels(as.factor(trk1$Bird))

trk1 <- trk1 %>% dplyr::filter(Bird == "Two") %>% droplevels()
levels(as.factor(trk1$id))
trk1
...
#' try one
st_area(hr_isopleths(dbbmm$Bastard_2016.12, level = 0.95)) / 1e6 #'  
...
#' test it here
intersection_1_95 <-
  st_intersection(hr_isopleths(dbbmm$Bastard_2016.12, level = 0.95),
