...
#' Tracks are in UTC now after I converted them
head(mydata)
mydata$time <-
...
time_difference <- 1440 #' the units relate to those set in difftime
...
tstep <- 86400 # 86400 secs = 24 hours
...
#' drop the level that's causing issues - AM269_2008/11
day_trk_mod <- day_trk_mod %>% 
  filter(identifier != "AM269_2008/11") %>% 
  droplevels()
...
kde_shape_1 <- trk_albers %>% filter(id == "AM269_2009/01") %>%
  hr_kde(., levels = c(0.95))
kde_shape_2 <- trk_albers %>% filter(id == "AM270_2009/01") %>%
  hr_kde(., levels = c(0.95))
...
data_summary <- read_csv(file = "summary/murn_data_month_summary.csv", col_names = T)
...
write.table(data_summary, "summary/murn_data_month_summary.csv", sep = ",", row.names=F)
