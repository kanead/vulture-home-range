...
filter(day_trk, id == "AM86_2005/12") %>% summarise(length = length(x_))
filter(day_trk, id == "AM86_2006/09") %>% summarise(length = length(x_))
...
kde_shape_1 <- trk2 %>% filter(id == "22959306_2016/04") %>%
  hr_kde(., levels = c(0.95))
kde_shape_2 <- trk2 %>% filter(id == "22959306_2016/05") %>%
  hr_kde(., levels = c(0.95))
...
#' writeRaster(kde_shape_1$ud, "KDE_105032_95_raw.tif")
#' writeRaster(kde_shape_2$ud, "KDE_105036_95_raw.tif")
...
data_summary$species  <- ifelse(data_summary$bird %in% cv, "cv", data_summary$species)
...
write.csv(data_summary, file = "summary/kerri_data_month_summary.csv", row.names = FALSE)
