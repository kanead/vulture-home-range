#' Mackenzie - identification of breeding sites
...
trk1_a <- trk1 %>% dplyr::filter(bird == "5403wb" | bird == "5404wb" | bird == "5784wb"
                                 | bird == "5785wb" | bird == "5789wb") %>% droplevels()

levels(as.factor(trk1_a$id))
trk1_a <- trk1_a %>% dplyr::select(x_ ,y_, t_, id)


trk1_b <- trk1 %>% dplyr::filter( bird == "5786wb" | bird == "5787wb" | bird == "5788wb" 
                                  | bird == "5863wb" | bird == "5864wb") %>% droplevels()
levels(as.factor(trk1_b$id))
trk1_b <- trk1_b %>% dplyr::select(x_ ,y_, t_, id)


trk1_c <- trk1 %>% dplyr::filter( bird == "6032wh" | bird == "6218wb" | bird == "6219wb" 
                                  | bird == "6220wh" | bird == "6484wh" | bird == "6485wh") %>% droplevels()
levels(as.factor(trk1_c$id))
trk1_c <- trk1_c %>% dplyr::select(x_ ,y_, t_, id)

#' imcrease the memory limit
...
# recursion_time$breeding <- if_else(recursion_time$time > 15, "yes", "no")
recursion_time$study <- "mackenzie"
recursion_time
write.csv(recursion_time, file = "summary/recursion_mackenzie.csv", row.names = FALSE)
