...
filter(day_trk, id == "WBV144782_2004/04") %>% summarise(length = length(x_))
filter(day_trk, id == "CV244781_2004/04") %>% summarise(length = length(x_))
...
kde_shape_1 <- trk2 %>% filter(id == "CV144780_2004/05") %>%
  hr_kde(., levels = c(0.95))
kde_shape_2 <- trk2 %>% filter(id == "CV144780_2004/06") %>%
  hr_kde(., levels = c(0.95))
...
write.csv(data_summary, file = "summary/mend_data_month_summary.csv", row.names = FALSE)
