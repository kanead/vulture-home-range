...
# write.csv(x = trk_albers, file = "regularised/lizanne_albers_reg_month.csv", row.names = FALSE)
...
kde_shape_1 <- trk_albers %>% filter(id == "VULT24_2013/05") %>%
  hr_kde(., levels = c(0.95))
kde_shape_2 <- trk_albers %>% filter(id == "VULT07_2013/05") %>%
  hr_kde(., levels = c(0.95))
...
data_summary <- read_csv("summary/lizanne_data_month_summary.csv", col_names = T)
...
write.table(data_summary, "summary/lizanne_data_month_summary.csv", sep = ",", row.names=F)
