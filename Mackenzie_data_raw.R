...
filter(day_trk, id == "5403wb") %>% summarise(length = length(x_))
filter(day_trk, id == "5404wb") %>% summarise(length = length(x_))
...
kde_shape_1 <- trk2 %>% filter(id == "5403wb") %>%
  hr_kde(., levels = c(0.95))
kde_shape_2 <- trk2 %>% filter(id == "5404wb") %>%
  hr_kde(., levels = c(0.95))
...
data_summary$species<-ifelse((data_summary$id == "6032wh") | (data_summary$id == "6220wh") | (data_summary$id == "6484wh") | (data_summary$id == "6485wh"), "wh", "wb")
data_summary

#' can export this data summary
write.csv(data_summary, file = "summary/mackenzie_data_summary.csv", row.names = FALSE)
