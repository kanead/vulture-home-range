...
####' Mendhelsohn tracks
####' Reg data by month
####' 9 Capes and 1 AWBV (possible hybrid)
...
# write.csv(x = trk_albers, file = "regularised/mend_albers_reg_month.csv", row.names = FALSE)
...
kde_shape_1 <- trk_albers %>% filter(id == "CV144780_2004/05") %>%
  hr_kde(., levels = c(0.95))
kde_shape_2 <- trk_albers %>% filter(id == "CV144780_2004/06") %>%
  hr_kde(., levels = c(0.95))
...
data_summary <- read_csv(file = "summary/mend_data_month_summary.csv", col_names = T)
...
write.table(data_summary, "summary/mend_data_month_summary.csv", sep = ",", row.names=F)
