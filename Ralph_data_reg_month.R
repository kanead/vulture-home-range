...
####' 4 individuals but using only 2
####' Regularised data
...
# write.csv(x = trk_albers, file = "regularised/ralph_albers_reg_month.csv", row.names = FALSE)
...
kde_shape_1 <- trk_albers %>% filter(id == "#399745060_2017/09") %>%
  hr_kde(., levels = c(0.95))
kde_shape_2 <- trk_albers %>% filter(id == "#399745060_2017/10") %>%
  hr_kde(., levels = c(0.95))
...
data_summary <- read_csv("summary/ralph_data_month_summary.csv", col_names = T)
...
write.table(data_summary, "summary/ralph_data_month_summary.csv", sep = ",", row.names=F)
