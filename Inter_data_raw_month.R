...
filter(day_trk, id == "Sanguza_2016/07") %>% summarise(length = length(x_))
filter(day_trk, id == "Tumaine_2016/07") %>% summarise(length = length(x_))
...
kde_shape_1 <- trk2 %>% filter(id == "Sanguza_2016/07") %>%
  hr_kde(., levels = c(0.95))
kde_shape_2 <- trk2 %>% filter(id == "Sanguza_2016/08") %>%
  hr_kde(., levels = c(0.95))
...
write.csv(data_summary, file = "summary/Inter_data_month_summary.csv", row.names = FALSE)
