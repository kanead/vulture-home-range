...
filter(day_trk, id == "1") %>% summarise(length = length(x_))
filter(day_trk, id == "2") %>% summarise(length = length(x_))
...
kde_shape_1 <- trk2 %>% filter(id == "1") %>%
  hr_kde(., levels = c(0.95))
kde_shape_2 <- trk2 %>% filter(id == "2") %>%
  hr_kde(., levels = c(0.95))
...
write.csv(data_summary, file = "summary/orr_data_summary.csv", row.names = FALSE)
