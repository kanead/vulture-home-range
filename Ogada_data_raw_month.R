...
filter(day_trk, id == "Bastard_2016/12") %>% summarise(length = length(x_))
filter(day_trk, id == "Chicken_2017/01") %>% summarise(length = length(x_))
...
kde_shape_1 <- trk2 %>% filter(id == "Ajali_2017/06") %>%
  hr_kde(., levels = c(0.95))
kde_shape_2 <- trk2 %>% filter(id == "Ajali_2017/07") %>%
  hr_kde(., levels = c(0.95))
...
write.csv(data_summary, file = "summary/ogada_data_month_summary.csv", row.names = FALSE)
