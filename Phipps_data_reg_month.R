...
write.csv(x = trk_albers, file = "regularised/phipps_albers_reg_month.csv", row.names = FALSE)
...
kde_shape_1 <- trk_albers %>% filter(id == "AG032_2009/04") %>%
  hr_kde(., levels = c(0.95))
kde_shape_2 <- trk_albers %>% filter(id == "AG032_2009/05") %>%
  hr_kde(., levels = c(0.95))
...
data_summary <- read_csv("summary/phipps_data_month_summary.csv", col_names = T)
...
write.table(data_summary, "summary/phipps_data_month_summary.csv", sep = ",", row.names=F)
