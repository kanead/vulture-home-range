...
kde_shape_1 <- trk_albers %>% filter(id == "101411") %>%
  hr_kde(., levels = c(0.95))
kde_shape_2 <- trk_albers %>% filter(id == "97872") %>%
  hr_kde(., levels = c(0.95))
...
data_summary <- read_csv("summary/murn_data_summary.csv", col_names = T)
...
write.table(data_summary, "summary/murn_data_summary.csv", sep = ",", row.names=F)
