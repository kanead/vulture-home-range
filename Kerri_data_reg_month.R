...
time_difference <- 8 #' the units relate to those set in difftime
...
# write.csv(x = trk_albers, file = "regularised/kerri_albers_reg_month.csv", row.names = FALSE)
...
kde_shape_1 <- trk_albers %>% filter(id == "22959306_2016/04") %>%
  hr_kde(., levels = c(0.95))
kde_shape_2 <- trk_albers %>% filter(id == "22959306_2016/05") %>%
  hr_kde(., levels = c(0.95))
...
data_summary <- read_csv(file = "summary/kerri_data_month_summary.csv", col_names = T)
...
write.table(data_summary, "summary/kerri_data_month_summary.csv", sep = ",", row.names=F)
