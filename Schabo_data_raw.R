...
filter(day_trk, id == "Cerhu") %>% summarise(length = length(x_))
filter(day_trk, id == "Emma") %>% summarise(length = length(x_))
...
kde_shape_1 <- trk2 %>% filter(id == "Cerhu") %>%
  hr_kde(., levels = c(0.95))
kde_shape_2 <- trk2 %>% filter(id == "Emma") %>%
  hr_kde(., levels = c(0.95))
...
write.csv(data_summary, file = "summary/schabo_data_summary.csv", row.names = FALSE)
