...
filter(day_trk, id == "128502") %>% summarise(length = length(x_))
filter(day_trk, id == "136377") %>% summarise(length = length(x_))
...
kde_shape_1 <- trk2 %>% filter(id == "128502") %>%
  hr_kde(., levels = c(0.95))
kde_shape_2 <- trk2 %>% filter(id == "136377") %>%
  hr_kde(., levels = c(0.95))
...
write.csv(data_summary, file = "summary/glynn_data_summary.csv", row.names = FALSE)
