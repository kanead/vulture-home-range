...
filter(day_trk, id == "AG032") %>% summarise(length = length(x_))
filter(day_trk, id == "AG094") %>% summarise(length = length(x_))
...
kde_shape_1 <- trk2 %>% filter(id == "AG032") %>%
  hr_kde(., levels = c(0.95))
kde_shape_2 <- trk2 %>% filter(id == "AG094") %>%
  hr_kde(., levels = c(0.95))
...
write.csv(data_summary, file = "summary/phipps_data_summary.csv", row.names = FALSE)
