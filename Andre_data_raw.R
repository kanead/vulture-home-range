...
filter(day_trk, id == "170948") %>% summarise(length = length(x_))
filter(day_trk, id == "170950") %>% summarise(length = length(x_))
...
kde_shape_1 <- trk2 %>% filter(id == "170948") %>%
  hr_kde(., levels = c(0.95))
kde_shape_2 <- trk2 %>% filter(id == "170950") %>%
  hr_kde(., levels = c(0.95))
...
write.csv(data_summary, file = "summary/andre_data_summary.csv", row.names = FALSE)
