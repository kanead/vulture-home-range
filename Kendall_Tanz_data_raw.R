...
filter(day_trk, id == "#109018692") %>% summarise(length = length(x_))
filter(day_trk, id == "#143101591") %>% summarise(length = length(x_))
...
kde_shape_1 <- trk2 %>% filter(id == "#109018692") %>%
  hr_kde(., levels = c(0.95))
kde_shape_2 <- trk2 %>% filter(id == "#143101591") %>%
  hr_kde(., levels = c(0.95))
...
data_summary$species <- max_time$species # "#333860073" "#333860224" hv, "#201940331" "#333858323" wh
...
write.csv(data_summary, file = "summary/kendall_tanz_data_summary.csv", row.names = FALSE)
