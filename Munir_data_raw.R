...
filter(day_trk, id == "171604adWBV05") %>% summarise(length = length(x_))
filter(day_trk, id == "171605Matira") %>% summarise(length = length(x_))
...
kde_shape_1 <- trk2 %>% filter(id == "171604adWBV05") %>%
  hr_kde(., levels = c(0.95))
kde_shape_2 <- trk2 %>% filter(id == "171605Matira") %>%
  hr_kde(., levels = c(0.95))
...
write.csv(data_summary, file = "summary/munir_data_summary.csv", row.names = FALSE)
