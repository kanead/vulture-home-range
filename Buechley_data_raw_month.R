...
####' Buechley tracks
####' Raw data month
####' 5 species, 28 IDs, 19 after I drop coarse data
...
####' load the data ----
data_path <- "data_buechley"   # path to the data
...
#' pull out the high res data
mydata <- mydata %>% filter(id != "LFVU02" & id != "VanirKamira" & id != "Bonez" &
                              id != "Gidget" & id != "Kemise" & id != "KingTut" &
                              id != "TomPetty" & id != "Walter" & id != "WBVU09")
...
filter(day_trk, id == "Timbavati_2018/10") %>% summarise(length = length(x_))
filter(day_trk, id == "JT_2018/11") %>% summarise(length = length(x_))
...
kde_shape_1 <- trk2 %>% filter(id == "Doug_2018/12") %>%
  hr_kde(., levels = c(0.95))
kde_shape_2 <- trk2 %>% filter(id == "Doug_2019/01") %>%
  hr_kde(., levels = c(0.95))
...
#' can extract species with the following
#' wb <- filter(mydata, species == "wb") %>% droplevels()

#' some bird IDs drop out because of the month filter process above

wb <- c("Doug", "Fringilla", "JT", "Lemba", "Mubanga", "Oscar", "Precision", "R.Speers",      "RogerMzeeKupe", "WBVU02", "WBVUSA01", "WBVUSA02")    
hv <- c("Johan", "Lizzy", "Odysseus", "Timbavati")
lf <- "M.C.Chops"
rv <- c("RUVU01", "Stratos")

data_summary$species  <- ifelse(data_summary$bird %in% wb, "wb", "other")
data_summary$species  <- ifelse(data_summary$bird %in% hv, "hv", data_summary$species)
data_summary$species  <- ifelse(data_summary$bird %in% lf, "lf", data_summary$species)
data_summary$species  <- ifelse(data_summary$bird %in% rv, "rv", data_summary$species)

data_summary$species

filter(data_summary, species == "rv")

#' can export this data summary
write.csv(data_summary, file = "summary/buechley_data_month_summary.csv", row.names = FALSE)
