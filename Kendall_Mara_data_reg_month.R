...
rv <- filter(mydata, species == "rv")
rv <- levels(as.factor(rv$id))
...
time_difference <- 480 #' the units relate to those set in difftime
...
tstep <- 14400 # 14400 secs = 4 hours
...
write.csv(x = trk_albers, file = "regularised/kendall_mara_albers_reg_month.csv", row.names = FALSE)
...
kde_shape_1 <- trk_albers %>% filter(id == "151329_2010/05") %>%
  hr_kde(., levels = c(0.95))
kde_shape_2 <- trk_albers %>% filter(id == "151329_2010/06") %>%
  hr_kde(., levels = c(0.95))
...
#' writeRaster(kde_shape_1$ud, "KDE_ID1_95_reg.tif")
#' writeRaster(kde_shape_2$ud, "KDE_ID2_95_reg.tif")

#####' export summary stats ----
#' combine the summary stats
data_summary <- read_csv(file = "summary/kendall_mara_data_month_summary.csv", col_names = T)
...
write.table(data_summary, "summary/kendall_mara_data_month_summary.csv", sep = ",", row.names=F)
