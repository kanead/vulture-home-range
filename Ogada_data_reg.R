...
time_difference <- 1 #' the units relate to those set in difftime
...
# write.csv(x = trk_albers, file = "regularised/ogada_albers_reg.csv", row.names = FALSE)
...
kde_shape_1 <- trk_albers %>% filter(id == "Ajali") %>%
  hr_kde(., levels = c(0.95))
kde_shape_2 <- trk_albers %>% filter(id == "Bastard") %>%
  hr_kde(., levels = c(0.95))
...
data_summary <- read_csv("summary/ogada_data_summary.csv", col_names = T)
data_summary$id
...
write.table(data_summary, "summary/ogada_data_summary.csv", sep = ",", row.names=F)
