...
filter(day_trk, id == "Doug") %>% summarise(length = length(x_))
filter(day_trk, id == "Fringilla") %>% summarise(length = length(x_))
...
kde_shape_1 <- trk2 %>% filter(id == "Doug") %>%
  hr_kde(., levels = c(0.95))
kde_shape_2 <- trk2 %>% filter(id == "Fringilla") %>%
  hr_kde(., levels = c(0.95))
...
write.csv(data_summary, file = "summary/buechley_data_summary.csv", row.names = FALSE)
