...
time_difference <- 90 #' the units relate to those set in difftime
...
# write.csv(x = trk_albers, file = "regularised/morgan_albers_reg_month.csv", row.names = FALSE)
...
kde_shape_1 <- trk_albers %>% filter(id == "X023_2013/06") %>%
  hr_kde(., levels = c(0.95))
kde_shape_2 <- trk_albers %>% filter(id == "X023_2013/07") %>%
  hr_kde(., levels = c(0.95))
...
data_summary <- read_csv("summary/morgan_data_month_summary.csv", col_names = T)
data_summary$id
...
write.table(data_summary, "summary/morgan_data_month_summary.csv", sep = ",", row.names=F)
