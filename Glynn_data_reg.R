...
time_difference <- 180 #' the units relate to those set in difftime
...
# write.csv(x = trk_albers, file = "regularised/glynn_albers_reg.csv", row.names = FALSE)
...
kde_shape_1 <- trk_albers %>% filter(id == "1120042") %>%
  hr_kde(., levels = c(0.95))
kde_shape_2 <- trk_albers %>% filter(id == "128502") %>%
  hr_kde(., levels = c(0.95))
...
kde_95 <- arrange(kde_95,as.numeric(id))
mcp_95 <- arrange(mcp_95,as.numeric(id))
kde_50 <- arrange(kde_50,as.numeric(id))
mcp_50 <- arrange(mcp_50,as.numeric(id))
ordered_prop <- res_prop[c("128502","136377","137296","1120042")]

L_95 <- L_95[c("128502","136377","137296","1120042")]
L_50 <- L_50[c("128502","136377","137296","1120042")]

data_summary <- read_csv("summary/glynn_data_summary.csv", col_names = T)
...
data_summary$overlap_reg <- unlist(ordered_prop)
...
write.table(data_summary, "summary/glynn_data_summary.csv", sep = ",", row.names=F)
