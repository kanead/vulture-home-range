...
#' Lizanne tracks have been converted to UTC by me
head(mydata)
mydata$time <-
...
filter(day_trk, id == "VULT24_2013/05") %>% summarise(length = length(x_))
filter(day_trk, id == "VULT07_2013/05") %>% summarise(length = length(x_))
...
kde_shape_1 <- trk2 %>% filter(id == "VULT01_2013/12") %>%
  hr_kde(., levels = c(0.95))
kde_shape_2 <- trk2 %>% filter(id == "VULT01_2014/02") %>%
  hr_kde(., levels = c(0.95))
...
write.csv(data_summary, file = "summary/lizanne_data_month_summary.csv", row.names = FALSE)
