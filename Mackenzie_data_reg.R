...
# write.csv(x = trk_albers, file = "regularised/mackenzie_albers_reg.csv", row.names = FALSE)
...
kde_shape_1 <- trk_albers %>% filter(id == "5403wb") %>%
  hr_kde(., levels = c(0.95))
kde_shape_2 <- trk_albers %>% filter(id == "5404wb") %>%
  hr_kde(., levels = c(0.95))
...
data_summary <- read_csv("summary/mackenzie_data_summary.csv", col_names = T)
data_summary$id
...
write.table(data_summary, "summary/mackenzie_data_summary.csv", sep = ",", row.names=F)
