...
filter(day_trk, id == "Ajali") %>% summarise(length = length(x_))
filter(day_trk, id == "Bastard") %>% summarise(length = length(x_))
...
kde_shape_1 <- trk2 %>% filter(id == "Ajali") %>%
  hr_kde(., levels = c(0.95))
kde_shape_2 <- trk2 %>% filter(id == "Bastard") %>%
  hr_kde(., levels = c(0.95))
...
write.csv(data_summary, file = "summary/ogada_data_summary.csv", row.names = FALSE)
