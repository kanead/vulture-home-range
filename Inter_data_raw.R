...
filter(day_trk, id == "Sanguza") %>% summarise(length = length(x_))
filter(day_trk, id == "Tumaine") %>% summarise(length = length(x_))
...
kde_shape_1 <- trk2 %>% filter(id == "Sanguza") %>%
  hr_kde(., levels = c(0.95))
kde_shape_2 <- trk2 %>% filter(id == "Tumaine") %>%
  hr_kde(., levels = c(0.95))
...
write.csv(data_summary, file = "summary/inter_data_summary.csv", row.names = FALSE)
