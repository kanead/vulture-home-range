...
# write.csv(x = trk_albers, file = "regularised/glynn_albers_reg_month.csv", row.names = FALSE)
...
kde_shape_1 <- trk_albers %>% filter(id == "1120042_2016/08") %>%
  hr_kde(., levels = c(0.95))
kde_shape_2 <- trk_albers %>% filter(id == "1120042_2016/09") %>%
  hr_kde(., levels = c(0.95))
...
data_summary <- read_csv(file = "summary/glynn_data_month_summary.csv", col_names = T)
...
write.table(data_summary, "summary/glynn_data_month_summary.csv", sep = ",", row.names=F)
