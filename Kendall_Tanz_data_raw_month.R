...
filter(day_trk, id == "#109018692_2015/10") %>% summarise(length = length(x_))
filter(day_trk, id == "#884521368_2015/10") %>% summarise(length = length(x_))
...
kde_shape_1 <- trk2 %>% filter(id == "#109018692_2015/10") %>%
  hr_kde(., levels = c(0.95))
kde_shape_2 <- trk2 %>% filter(id == "#143101591_2016/03") %>%
  hr_kde(., levels = c(0.95))
...
data_summary$species <- "wb" # need to add these later
data_summary$study <- "kendall_tanz"
...
write.csv(data_summary, file = "summary/kendall_tanz_data_month_summary.csv", row.names = FALSE)
