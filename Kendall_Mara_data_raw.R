...
filter(day_trk, id == "151329") %>% summarise(length = length(x_))
filter(day_trk, id == "AG082") %>% summarise(length = length(x_))
...
kde_shape_1 <- trk2 %>% filter(id == "151329") %>%
  hr_kde(., levels = c(0.95))
kde_shape_2 <- trk2 %>% filter(id == "AG082") %>%
  hr_kde(., levels = c(0.95))
...
write.csv(data_summary, file = "summary/kendall_mara_data_summary.csv", row.names = FALSE)
