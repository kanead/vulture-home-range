...
kde_shape_1 <- trk_albers %>% filter(id == "Doug_2018/12") %>%
  hr_kde(., levels = c(0.95))
kde_shape_2 <- trk_albers %>% filter(id == "Doug_2019/01") %>%
  hr_kde(., levels = c(0.95))
...
data_summary <- read_csv(file = "summary/buechley_data_month_summary.csv", col_names = T)
...
write.table(data_summary, "summary/buechley_data_month_summary.csv", sep = ",", row.names=F)
