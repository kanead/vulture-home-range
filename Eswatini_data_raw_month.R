...
filter(day_trk, id == "ID1_2015/06") %>% summarise(length = length(x_))
filter(day_trk, id == "ID2_2015/06") %>% summarise(length = length(x_))
...
kde_shape_1 <- trk2 %>% filter(id == "ID1_2015/06") %>%
  hr_kde(., levels = c(0.95))
kde_shape_2 <- trk2 %>% filter(id == "ID1_2015/07") %>%
  hr_kde(., levels = c(0.95))
...
write.csv(data_summary, file = "summary/eswatini_data_month_summary.csv", row.names = FALSE)
