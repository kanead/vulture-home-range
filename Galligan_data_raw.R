...
filter(day_trk, id == "lf143660") %>% summarise(length = length(x_))
filter(day_trk, id == "wb143657") %>% summarise(length = length(x_))
...
kde_shape_1 <- trk2 %>% filter(id == "lf143660") %>%
  hr_kde(., levels = c(0.95))
kde_shape_2 <- trk2 %>% filter(id == "wb143657") %>%
  hr_kde(., levels = c(0.95))
...
write.csv(data_summary, file = "summary/galligan_data_summary.csv", row.names = FALSE)
