...
time_difference <- 480 #' the units relate to those set in difftime, 8 hours in this case
...
# write.csv(x = trk_albers, file = "regularised/kendall_mara_albers_reg.csv", row.names = FALSE)
...
kde_shape_1 <- trk_albers %>% filter(id == "151329") %>%
  hr_kde(., levels = c(0.95))
kde_shape_2 <- trk_albers %>% filter(id == "AG082") %>%
  hr_kde(., levels = c(0.95))
...
data_summary <- read_csv("summary/kendall_mara_data_summary.csv", col_names = T)
...
write.table(data_summary, "summary/kendall_mara_data_summary.csv", sep = ",", row.names=F)
