...
filter(day_trk, id == "Heidi_2013/02") %>% summarise(length = length(x_))
filter(day_trk, id == "Sascha_2013/02") %>% summarise(length = length(x_))
...
kde_shape_1 <- trk2 %>% filter(id == "Emma_2016/02") %>%
  hr_kde(., levels = c(0.95))
kde_shape_2 <- trk2 %>% filter(id == "Emma_2016/03") %>%
  hr_kde(., levels = c(0.95))
...
write.csv(data_summary, file = "summary/schabo_data_month_summary.csv", row.names = FALSE)
