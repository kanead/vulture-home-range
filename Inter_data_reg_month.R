...
#' pull out the names for use in export
...
time_difference <- 2 #' the units relate to those set in difftime
...
# write.csv(x = trk_albers, file = "regularised/inter_albers_reg_month.csv", row.names = FALSE)
...
kde_shape_1 <- trk_albers %>% filter(id == "Sanguza_2016/07") %>%
  hr_kde(., levels = c(0.95))
kde_shape_2 <- trk_albers %>% filter(id == "Sanguza_2016/08") %>%
  hr_kde(., levels = c(0.95))
...
#' writeRaster(kde_shape_1$ud, "KDE_105032_95_reg.tif")
#' writeRaster(kde_shape_2$ud, "KDE_105036_95_reg.tif")

#####' export summary stats ----
#' combine the summary stats
data_summary <- read_csv("summary/Inter_data_month_summary.csv", col_names = T)
...
#' add the species
data_summary$species  <- ifelse(data_summary$bird %in% wh, "wh", "wb")
data_summary$species

#' can export this data summary
#' append the raw data file
write.table(data_summary, "summary/Inter_data_month_summary.csv", sep = ",", row.names=F)
