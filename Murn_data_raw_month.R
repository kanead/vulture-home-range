...
filter(day_trk, id == "AM269_2008/11") %>% summarise(length = length(x_))
filter(day_trk, id == "AM269_2009/01") %>% summarise(length = length(x_))
...
kde_shape_1 <- trk2 %>% filter(id == "AM269_2008/11") %>%
  hr_kde(., levels = c(0.95))
kde_shape_2 <- trk2 %>% filter(id == "AM269_2009/01") %>%
  hr_kde(., levels = c(0.95))
...
data_summary
#' AM269_2008/11 is weird and producing a tiny home range
#' can remove it
data_summary <- data_summary %>% filter(id != "AM269_2008/11")
data_summary
#' can export this data summary
write.csv(data_summary, file = "summary/murn_data_month_summary.csv", row.names = FALSE)
