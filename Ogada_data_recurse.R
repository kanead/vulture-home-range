...
trk1_a <- trk1 %>% dplyr::filter(bird == "Ajali" | bird == "Bastard") %>% droplevels()

levels(as.factor(trk1_a$id))
trk1_a <- trk1_a %>% dplyr::select(x_ ,y_, t_, id)


trk1_b <- trk1 %>% dplyr::filter( bird == "Chicken" | bird == "Easy") %>% droplevels()
levels(as.factor(trk1_b$id))
trk1_b <- trk1_b %>% dplyr::select(x_ ,y_, t_, id)


trk1_c <- trk1 %>% dplyr::filter( bird == "Francos" | bird == "Melako") %>% droplevels()
...
recursion_time
write.csv(recursion_time, file = "summary/recursion_ogada.csv", row.names = FALSE)
