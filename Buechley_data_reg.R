...
# write.csv(x = trk_albers, file = "regularised/buechley_albers_reg.csv", row.names = FALSE)
...
kde_shape_1 <- trk_albers %>% filter(id == "Doug") %>%
  hr_kde(., levels = c(0.95))
kde_shape_2 <- trk_albers %>% filter(id == "Fringilla") %>%
  hr_kde(., levels = c(0.95))
...
data_summary <- read_csv("summary/buechley_data_summary.csv", col_names = T)
(data_summary$id)
...
write.table(data_summary, "summary/buechley_data_summary.csv", sep = ",", row.names=F)
