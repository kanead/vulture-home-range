...
filter(day_trk, id == "22959306") %>% summarise(length = length(x_))
filter(day_trk, id == "27230695") %>% summarise(length = length(x_))
...
kde_shape_1 <- trk2 %>% filter(id == "22959306") %>%
  hr_kde(., levels = c(0.95))
kde_shape_2 <- trk2 %>% filter(id == "27233665") %>%
  hr_kde(., levels = c(0.95))
...
#' writeRaster(kde_shape_1$ud, "KDE_#109018692_95_raw.tif")
#' writeRaster(kde_shape_2$ud, "KDE_#143101591_95_raw.tif")
...
data_summary$species  <- ifelse(data_summary$id %in% wb, "wb", "other")
data_summary$species  <- ifelse(data_summary$id %in% cv, "cv", data_summary$species)
data_summary$species  <- ifelse(data_summary$id %in% lf, "lf", data_summary$species)
data_summary$species

#' can export this data summary
write.csv(data_summary, file = "summary/kerri_data_summary.csv", row.names = FALSE)
