...
filter(day_trk, id == "AG082_2009/08") %>% summarise(length = length(x_))
filter(day_trk, id == "AG084_2009/08") %>% summarise(length = length(x_))
...
kde_shape_1 <- trk2 %>% filter(id == "151329_2010/05") %>%
  hr_kde(., levels = c(0.95))
kde_shape_2 <- trk2 %>% filter(id == "151329_2010/06") %>%
  hr_kde(., levels = c(0.95))
...
data_summary$study <- "kendall_mara"
...
write.csv(data_summary, file = "summary/kendall_mara_data_month_summary.csv", row.names = FALSE)
