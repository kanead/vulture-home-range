...
kde_shape_1 <- trk_albers %>% filter(id == "Sanguza") %>%
  hr_kde(., levels = c(0.95)) 
kde_shape_2 <- trk_albers %>% filter(id == "Tumaine") %>%
  hr_kde(., levels = c(0.95))
...
#ordered_prop <- res_prop[c("105032","105036","105039","105040","105043","105390")]

data_summary <- read_csv("summary/inter_data_summary.csv", col_names = T)
...
write.table(data_summary, "summary/inter_data_summary.csv", sep = ",", row.names=F)
