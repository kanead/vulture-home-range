...
filter(day_trk, id == "#399745060") %>% summarise(length = length(x_))
filter(day_trk, id == "#399749318") %>% summarise(length = length(x_))
...
kde_shape_1 <- trk2 %>% filter(id == "#399745060") %>%
  hr_kde(., levels = c(0.95))
kde_shape_2 <- trk2 %>% filter(id == "#399749318") %>%
  hr_kde(., levels = c(0.95))
...
write.csv(data_summary, file = "summary/ralph_data_summary.csv", row.names = FALSE)
