...
filter(day_trk, id == "3_2008/04") %>% summarise(length = length(x_))
filter(day_trk, id == "4_2008/04") %>% summarise(length = length(x_))
...
kde_shape_1 <- trk2 %>% filter(id == "1_2008/05") %>%
  hr_kde(., levels = c(0.95))
kde_shape_2 <- trk2 %>% filter(id == "1_2008/06") %>%
  hr_kde(., levels = c(0.95))
...
#' writeRaster(kde_shape_1$ud, "KDE_1_95_raw.tif")
#' writeRaster(kde_shape_2$ud, "KDE_2_95_raw.tif")
...
write.csv(data_summary, file = "summary/orr_data_month_summary.csv", row.names = FALSE)
