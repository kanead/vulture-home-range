...
filter(day_trk, id == "101411") %>% summarise(length = length(x_))
filter(day_trk, id == "97872") %>% summarise(length = length(x_))
...
kde_shape_1_95 <- trk2 %>% filter(id == "101411") %>%
  hr_kde(., levels = c(0.95))
kde_shape_2_95 <- trk2 %>% filter(id == "97872") %>%
  hr_kde(., levels = c(0.95))
...
write.csv(data_summary, file = "summary/murn_data_summary.csv", row.names = FALSE)
