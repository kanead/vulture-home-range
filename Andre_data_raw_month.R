...
filter(day_trk, id == "170950_2017/07") %>% summarise(length = length(x_))
filter(day_trk, id == "170950_2017/08") %>% summarise(length = length(x_))
...
kde_shape_1 <- trk2 %>% filter(id == "170948_2018/01") %>%
  hr_kde(., levels = c(0.95))
kde_shape_2 <- trk2 %>% filter(id == "170950_2017/07") %>%
  hr_kde(., levels = c(0.95))
...
data_summary
#' 170950_2018/01 is weird and producing a tiny home range
#' can remove it
data_summary <- data_summary %>% filter(id != "170950_2018/01")
#' can export this data summary
write.csv(data_summary, file = "summary/andre_data_month_summary.csv", row.names = FALSE)
