...
# write.csv(x = trk_albers, file = "regularised/spiegel_albers_reg_month.csv", row.names = FALSE)
...
kde_shape_1 <- trk_albers %>% filter(id == "1_2008/05") %>%
  hr_kde(., levels = c(0.95))
kde_shape_2 <- trk_albers %>% filter(id == "1_2008/06") %>%
  hr_kde(., levels = c(0.95))
...
data_summary <- read_csv(file = "summary/orr_data_month_summary.csv", col_names = T)
#' check the order of the ids
head(data_summary$id)
tail(data_summary$id)
...
data_summary$species<-ifelse((data_summary$bird == "2") | (data_summary$bird == "11") | (data_summary$bird == "12"), "lf", "wb")


#' can export this data summary
#' append the raw data file
write.table(data_summary, "summary/orr_data_month_summary.csv", sep = ",", row.names=F)
