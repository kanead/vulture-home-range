...
####' Andre tracks - Andre Botha
####' 2 individuals
####' Regularised data monthly

#' 6/19/2017	African White-backed Vulture	Immature	Unknown	18.89045S	034.39579E	Loop 4, Gorongosa National Park	G33794	Green	G006	PTT: 1780001
#' 6/22/2017	African White-backed Vulture	Sub-adult	Unknown	18.938955S	034.392303E	Loop 3, Gorongosa National Park	G33753	Green	G007	PTT: 178008
...
####' load the data ----
data_path <- "data_andre"   # path to the data
...
#' remove points before deployment 
mydata <- mydata %>% filter(time >= "2017-06-22")
...
#' drop the level that's causing issues - 170950_2018/01
day_trk_mod <- day_trk_mod %>% 
  filter(identifier != "170950_2018/01") %>% 
  droplevels()
...
write.csv(x = trk_albers, file = "regularised/andre_albers_reg_month.csv", row.names = FALSE)
...
kde_shape_1 <- trk_albers %>% filter(id == "170948_2018/01") %>%
  hr_kde(., levels = c(0.95))
kde_shape_2 <- trk_albers %>% filter(id == "170950_2017/07") %>%
  hr_kde(., levels = c(0.95))
...
#' writeRaster(kde_shape_1$ud, "KDE_170948_95_reg.tif")
#' writeRaster(kde_shape_2$ud, "KDE_170948_95_reg.tif")

#####' export summary stats ----
#' combine the summary stats
data_summary <- read_csv("summary/andre_data_month_summary.csv", col_names = T)
...
write.table(data_summary, "summary/andre_data_month_summary.csv", sep = ",", row.names=F)
