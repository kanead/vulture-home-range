...
filter(day_trk, id == "CV144780") %>% summarise(length = length(x_))
filter(day_trk, id == "CV244781") %>% summarise(length = length(x_))
...
kde_shape_1 <- trk2 %>% filter(id == "CV144780") %>%
  hr_kde(., levels = c(0.95))
kde_shape_2 <- trk2 %>% filter(id == "CV244781") %>%
  hr_kde(., levels = c(0.95))
...
write.csv(data_summary, file = "summary/mend_data_summary.csv", row.names = FALSE)
