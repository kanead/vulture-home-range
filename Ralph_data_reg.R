...
head(mydata)
mydata$time <-
  as.POSIXct(mydata$time, format = "%d/%m/%Y %H:%M:%S", tz = "UTC")
head(mydata)
...
# write.csv(x = trk_albers, file = "regularised/ralph_albers_reg.csv", row.names = FALSE)
...
kde_shape_1 <- trk_albers %>% filter(id == "#399745060") %>%
  hr_kde(., levels = c(0.95))
kde_shape_2 <- trk_albers %>% filter(id == "#399749318") %>%
  hr_kde(., levels = c(0.95))
...
data_summary <- read_csv("summary/ralph_data_summary.csv", col_names = T)
...
write.table(data_summary, "summary/ralph_data_summary.csv", sep = ",", row.names=F)
