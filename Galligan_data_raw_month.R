...
filter(day_trk, id == "lf143660_2015/04") %>% summarise(length = length(x_))
filter(day_trk, id == "lf143660_2015/05") %>% summarise(length = length(x_))
...
kde_shape_1 <- trk2 %>% filter(id == "lf143660_2015/04") %>%
  hr_kde(., levels = c(0.95))
kde_shape_2 <- trk2 %>% filter(id == "lf143660_2015/05") %>%
  hr_kde(., levels = c(0.95))
...
data_summary$species<-ifelse((data_summary$bird == "lf143660"), "lf", "wb")
data_summary

#' can export this data summary
write.csv(data_summary, file = "summary/galligan_data_month_summary.csv", row.names = FALSE)
